...
const Referencias = () => {
  ...
  return (
    <div className="bg-white p-3 relative">
      <div className="w-full sm:w-2/3 lg:w-1/3 mx-auto" style={{ scrollSnapType: 'x mandatory' }}>
        {carouselItems.map((item, index) => (
          <div key={index} style={{ display: currentSlide === index ? 'block' : 'none' }}>
            <input className="sr-only peer" type="radio" name="carousel" id={`carousel-${index + 1}`} checked={currentSlide === index} readOnly />
            <div className="w-full bg-white rounded-lg shadow-lg transition-all duration-300 opacity-0 peer-checked:opacity-100 peer-checked:z-10 z-0">
              ...
              <div className="absolute top-1/2 w-full flex justify-between px-4">
                <button onClick={handlePrevSlide} className="inline-block text-red-600 cursor-pointer bg-white rounded-full shadow-md active:translate-y-0.5">
                  &lt;
                </button>
                <button onClick={handleNextSlide} className="relative text-red-600 cursor-pointer bg-white rounded-full shadow-md active:translate-y-0.5">
                  &gt;
                </button>
              </div>
            </div>
